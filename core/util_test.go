...
func BatchNewDrand(t *testing.T, n int, insecure bool, scheme scheme.Scheme, opts ...ConfigOption) (
	...
	if insecure {
		...
		privs, group = test.BatchTLSIdentities(n, scheme)
	}
...
// specified period
func NewDrandTestScenario(t *testing.T, n, thr int, period time.Duration, scheme scheme.Scheme) *DrandTestScenario {
	dt := new(DrandTestScenario)

	drands, _, dir, certPaths := BatchNewDrand(
		t, n, false, scheme, WithCallOption(grpc.WaitForReady(true)),
	)

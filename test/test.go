...
func BatchIdentities(n int, scheme scheme.Scheme) ([]*key.Pair, *key.Group) {
	privs := GenerateIDs(n)
	thr := key.MinimumT(n)
	...
	group := key.LoadGroup(ListFromPrivates(privs), 1, dp, 30*time.Second, 0, scheme)
	group.Threshold = thr
	return privs, group
}

// BatchTLSIdentities generates n secure (TLS) identities
func BatchTLSIdentities(n int, scheme scheme.Scheme) ([]*key.Pair, *key.Group) {
	pairs, group := BatchIdentities(n, scheme)
	for i := 0; i < n; i++ {
